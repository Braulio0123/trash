                ...
                if (!respuesta) {
                    return caches.match(event.request).then(cachedResponse => {
                        if (cachedResponse) {
                            ...
                            return caches.match('/images/maxwell_cat.jpg');
                        }
                    });
                } else {
...
self.addEventListener('push', event => {
    ...
    const title = payload.title || 'Nueva notificación basura';
    const options = {
      body: payload.body || 'Tienes una nueva basura de notificación',
    };
...
self.addEventListener('fetch', event => {
    if (event.request.url.includes('https://trash-server-8bca.onrender.com/api/users/create-user')) {
        event.respondWith(
